...
#define PWM_GREEN_PERIOD_NS	DT_PWMS_PERIOD(PWM_GREEN_LED_NODE)
...
#define PWM_BLUE_PERIOD_NS	DT_PWMS_PERIOD(PWM_BLUE_LED_NODE)
...
int leds_init(void)
{
	...
	ret = pwm_set(PWM_GREEN_DEVICE, PWM_GREEN_CHANNEL,
		      PWM_GREEN_PERIOD_NS, 0U, PWM_GREEN_FLAGS);
	if (ret != 0) {
	...
	ret = pwm_set(PWM_BLUE_DEVICE, PWM_BLUE_CHANNEL,
		      PWM_BLUE_PERIOD_NS, 0U, PWM_BLUE_FLAGS);
	if (ret != 0) {
...
int leds_set(led_t led, led_state_t state)
{
	switch (led) {
	case LED_GREEN:
		return leds_set_blinking_phase(led, PWM_GREEN_PERIOD_NS,
					       state == LED_OFF ? 0U : PWM_GREEN_PERIOD_NS);
	case LED_BLUE:
		return leds_set_blinking_phase(led, PWM_BLUE_PERIOD_NS,
					       state == LED_OFF ? 0U : PWM_BLUE_PERIOD_NS);
	case LED_RED:
